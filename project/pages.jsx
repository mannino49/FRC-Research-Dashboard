// Collaborator directory + New project form
function People({ projects, onOpenProject }) {
  const all = Object.values(window.PEOPLE).filter(p => p.kind === 'external');
  function projectsOf(id) { return projects.filter(p => p.coauthors.includes(id)); }
  function activeOf(id) { return projects.filter(p => p.coauthors.includes(id) && p.turn === id); }
  ...
  return (
    <section className="people">
      ...
      {all.map(pr => {
        const projs = projectsOf(pr.initials === 'KF' ? 'KF'
                      : pr.initials === 'JK' ? 'JK'
                      : pr.initials === 'SS' ? 'SS'
                      : pr.initials === 'GN' ? 'GN'
                      : pr.initials === 'HS' ? 'HS' : '');
        // find id key
        const id = Object.entries(window.PEOPLE).find(([k,v]) => v === pr)[0];
        ...
        return (
          <div className="person" key={id} onClick={() => ps[0] && onOpenProject(ps[0].id)}>
            ...
            <span className="active">
              {acts.length > 0
                ? <span className="now">→ their turn on {acts.length}</span>
...
function NewProject({ onCreate, onCancel }) {
  ...
  const externals = Object.entries(window.PEOPLE).filter(([,p]) => p.kind === 'external');
    ...
    <div className="form-wrap">
      ...
      <div className="field">
        ...
        <div className="pillrow">
          {TYPES.map(t => (
            <button key={t} className={type === t ? 'on' : ''} onClick={() => setType(t)}>{typeMark(t)} {typeWord(t)}</button>
          ))}
        </div>
      </div>
      ...
      <div className="field">
        ...
        <div className="pillrow">
          {externals.map(([id, p]) => (
            <button key={id} className={coauthors.includes(id) ? 'on' : ''} onClick={() => toggle(id)}>
              {p.name}
            </button>
          ))}
        </div>
...
Object.assign(window, { People, NewProject });
